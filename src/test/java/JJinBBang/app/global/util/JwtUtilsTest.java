...
import JJinBBang.app.global.jwt.service.impl.AccessTokenServiceImpl;
import JJinBBang.app.global.jwt.service.impl.RefreshTokenServiceImpl;
import JJinBBang.app.global.jwt.service.impl.SignupTokenServiceImpl;
...
class JwtUtilsTest {
	...
	@Mock TokenService accessTokenService;
	@Mock TokenService refreshTokenService;
	@Mock TokenService signupTokenService;
	...
	void realToken_parseClaimsAndGetProviderAndStatus() {
		...
		var accessSvc  = new AccessTokenServiceImpl(SECRET_KEY, 60_000L);
		var refreshSvc = new RefreshTokenServiceImpl(
				SECRET_KEY,
				60_000L,
				new InMemoryRefreshTokenRepository()
		);
		var signupSvc  = new SignupTokenServiceImpl(SECRET_KEY, 60_000L);

		// 2) JwtUtils 인스턴스 생성
		var realJwtUtils = new JwtUtils(
				SECRET_KEY, accessSvc, refreshSvc, signupSvc
		);
	...
	void validateToken_expired_Throws() throws InterruptedException {
		// 1초 만료 서비스
		var accessSvc  = new AccessTokenServiceImpl(SECRET_KEY, 1_000L);
		var refreshSvc = new RefreshTokenServiceImpl(
				SECRET_KEY,
				1_000L,
				new InMemoryRefreshTokenRepository()
		);
		var signupSvc  = new SignupTokenServiceImpl(SECRET_KEY, 1_000L);
